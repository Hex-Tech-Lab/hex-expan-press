...
Trend-harvest + digital-product qualification pipeline. Flat repo, public GitHub repo under the org (`github.com/Hex-Tech-Lab/hex-expan-press` — moved 2026-09-18 from a stray personal repo at `TechHypeXP/hex-expan`; public because CodeRabbit/Cubic PR-review tools need it on their free tiers; `.env`, `data/`, `downloads/` all gitignored). All GitHub work for this project goes through the `Hex-Tech-Lab` org, never a personal account. TypeScript-only, run via `pnpm exec tsx` (npm is disabled on this machine — pnpm only). Test suite: Vitest, `pnpm test` (`payments/src/ledger.ts`, `payments/src/pricing_tier_cascade.ts`, `payments/src/redis.ts` covered; `payments/src/provider_router.ts` has no test yet — see naming note below).

**Naming convention (added 2026-09-22, fixes a real collision):** `payments/src/pricing_tier_cascade.ts` (auto price-tier drops on refund-rate tripwire) and `payments/src/provider_router.ts` (weighted/sequential multi-provider distribution + failover, built but unwired — no config, no caller, no test yet) both used to be informally called "cascade" — never call either one just "cascade" in prose or new code; use the full filename-derived name so the two unrelated systems can't be confused again.

## Agent Routing & Roles (added 2026-09-22 — binding, replaces any session-verbal routing statement)

- **OC (OpenCode):** locked to `z-ai/glm-5.3-flash`, **low effort** by default (raise effort only if a task demonstrably needs it). Role: strict coding, mechanical execution, pipeline/script generation. Provider pin for this model is CoreWeave (see `opencode.json` and the cost note in Hard-won quirks below) — do not change without a cost note.
- **AGY (Antigravity):** Gemini family, variable effort (Flash 3.8 no/low-effort default; 3.1 Pro with any reasoning effort for harder work). Role: UI/presentation work, visual QA, architectural review. Gemini family is preferred specifically for UI/presentation tasks.
- **CCT / CC (Claude):** vertical ownership — orchestrator, final auditor, verifier, repository manager. Assigns work to OC/AGY by their strengths above, may run them in parallel, but must personally verify their outputs (render/inspect actual artifacts) rather than relay their self-reports. See Multi-Agent Ledger Protocol below for the logging discipline that goes with this.

## Book production pipeline & quality gates (ADR 0043, 0044)
Order is fixed: `manuscript.md` → **Literary Quality Gate** (`scripts/literary_metrics.py` + AGY read-only assessment → `data/intel/duane_book/qa/literary_assessment.md`; accuracy contradictions fail outright) → build (`scripts/chapter_proof.sh <Ch> <NextCh>`) → **layout QA gate** (`scripts/book_qa.py`, rules + design tokens in `data/intel/duane_book/qa/design_rules.json`, which `template.typ` reads directly) → **visual regression** (`scripts/visual_regress.py` against founder-approved baselines) → founder sign-off, then `--approve` the new baseline. Work one chapter at a time. A chapter is never handed to the founder with a failing gate. Every new defect class gets a rule.

## Multi-Agent Ledger Protocol (added 2026-09-18 — mandatory when more than one agent/session works this repo concurrently)

This project is sometimes worked by more than one agent session at once (e.g. this session alongside a separate AGY/Antigravity or opencode session). A real collision already happened once: two sessions independently wrote near-identical fixes to the same function in `payments/bake_creator_pages.ts` without either knowing about the other. To prevent silent conflicts or duplicated work:

1. **Before starting any file edit**, read `docs/agent_ledger.jsonl` (append-only, newest last) to see what other agents have done or are doing in this repo recently. If another agent's most recent entry touches the same file/area you're about to change, stop and reconcile before proceeding — do not silently overwrite or duplicate.
2. **Log every discrete action at tick level** (not just a session summary at the end) by appending one JSON line per action to `docs/agent_ledger.jsonl`: `{"ts": "<ISO-8601>", "agent": "<model/tool identity, e.g. 'claude-sonnet-5' or 'agy-antigravity'>", "action": "<short verb phrase>", "files": ["<paths touched>"], "note": "<one line>"}`. Log the action as you take it, not retroactively in a batch at session end — retroactive summaries are exactly what produced the unverifiable/inaccurate claims found in the 2026-09-18 audit (see `docs/creator-lifecycle-and-samples.md` history / session record for that incident).
3. **Self-report accuracy matters.** A prior handover falsely claimed a `package.json` change that was never actually made — self-reports must be checked against the actual file state, not asserted from memory of intent.
...
- **OR app attribution (standing rule 2026-09-16):** every OpenRouter call from house code sends `X-Title: hex-expan` + `HTTP-Referer: https://github.com/TechHypeXP/hex-expan` — scripts/engines/council/one-off .cjs all patched; new scripts must include them. opencode sessions attribute themselves; Handy (dictation) calls may show Unknown (user-side app). Provider pin: glm-5.3-flash = **Morph first, then Together, CoreWeave, Relace — no fallbacks** (updated 2026-09-27 by user mandate, supersedes the 2026-09-22 CoreWeave-only pin) via `@preset/glm-53-flash-on-cheap` (global opencode.json + hex-yt-intel) and explicit `provider: {order:['morph','together','coreweave','relace'], only:[...same], allow_fallbacks:false}` (project opencode.json files). Verified live 2026-09-27: morph endpoint 200 with correct content. **Cost note (2026-09-27):** Morph is cheapest for this workload (test call cost ~$0.000029 for 68 tokens) and the preset sorts by latency; order preserves Together before CoreWeave as cheaper-than-CoreWeave secondaries, all within `allow_fallbacks:false` so a provider outage retries same-provider and fails loud. **Morph quirk (live-proven 2026-09-27):** glm on morph returns `content: null` unless `max_tokens >= ~2500` (reasoning tokens eat the budget) — raw-API call sites MUST use the large max_tokens + content-shape validation. Config changes need an opencode restart to take effect on running sessions. Standing rule remains: never pin a different provider/model in a project opencode.json without a cost note like this one. **Raw-API provider pin (standing rule 2026-09-16, updated 2026-09-27):** the opencode.json pin governs ONLY the CLI — every script calling OR's HTTP API directly MUST set `provider: { order: [...], allow_fallbacks: false }` per model family (glm → `['morph','together','coreweave','relace']` in that order, no fallbacks, per the 2026-09-27 mandate; exclusive first-party models → their own slug: `perplexity/sonar`→`['perplexity']`, `x-ai/grok-4.3`→`['x-ai']`, `anthropic/*`→`['anthropic']`, `deepseek/*`→`['deepseek']`; verify slugs first via `GET /api/v1/models/<model>/endpoints`). **Response validation (standing rule 2026-09-16):** every raw-OR call site validates the CONTENT shape before accepting (HTTP 200 ≠ success — e.g. certify vision must parse OVERLAP:yes/no; harvest/creator_scan/trends must parse the demanded JSON array; research .cjs must return non-empty prose) and retries the SAME provider with backoff, max 3 attempts, then fails loud per that file's discipline (throw → incomplete_reasons / FAIL marker / indeterminate=FAIL). Two live traps: relace's glm deployment 400s image input without a system prompt (include one), and OR does NOT fall through 400-class errors intra-list. Exception: `/audio/transcriptions` (STT) IGNORES the `provider` field entirely — no pin possible, proven empirically.
