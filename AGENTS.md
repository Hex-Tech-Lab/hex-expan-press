...
- **OR app attribution (standing rule 2026-09-16):** every OpenRouter call from house code sends `X-Title: hex-expan` + `HTTP-Referer: https://github.com/TechHypeXP/hex-expan` — scripts/engines/council/one-off .cjs all patched; new scripts must include them. opencode sessions attribute themselves; Handy (dictation) calls may show Unknown (user-side app). Provider pin: glm-5.3-flash = **CoreWeave** (updated 2026-09-22, supersedes the 2026-09-16 Relace pin below) via opencode.json provider.options (global + all project configs); config changes need an opencode restart to take effect on running sessions. **Cost note (2026-09-22):** CoreWeave is retained over Relace despite being ~1.4x more expensive because it is ~2.5x faster — net win for OC's rapid low-effort execution loops where wall-clock turnaround matters more than per-call cost. Standing rule remains: never pin a different provider/model in a project opencode.json without a cost note like this one. **Raw-API provider pin (standing rule 2026-09-16, provider updated 2026-09-22):** the opencode.json pin governs ONLY the CLI — every script calling OR's HTTP API directly MUST set `provider: { order: [...], allow_fallbacks: false }` per model family (glm → `['coreweave']` ONLY, per the 2026-09-22 cost note above; a provider outage retries same-provider and then fails loud, never re-routes; exclusive first-party models → their own slug: `perplexity/sonar`→`['perplexity']`, `x-ai/grok-4.3`→`['x-ai']`, `anthropic/*`→`['anthropic']`, `deepseek/*`→`['deepseek']`; verify slugs first via `GET /api/v1/models/<model>/endpoints`). **Response validation (standing rule 2026-09-16):** every raw-OR call site validates the CONTENT shape before accepting (HTTP 200 ≠ success — e.g. certify vision must parse OVERLAP:yes/no; harvest/creator_scan/trends must parse the demanded JSON array; research .cjs must return non-empty prose) and retries the SAME provider with backoff, max 3 attempts, then fails loud per that file's discipline (throw → incomplete_reasons / FAIL marker / indeterminate=FAIL). Two live traps: relace's glm deployment 400s image input without a system prompt (include one), and OR does NOT fall through 400-class errors intra-list. Exception: `/audio/transcriptions` (STT) IGNORES the `provider` field entirely — no pin possible, proven empirically.
