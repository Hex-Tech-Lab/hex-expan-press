...
Trend-harvest + digital-product qualification pipeline. Flat repo, public GitHub repo under the org (`github.com/Hex-Tech-Lab/hex-expan-press` — moved 2026-09-18 from a stray personal repo at `TechHypeXP/hex-expan`; public because CodeRabbit/Cubic PR-review tools need it on their free tiers; `.env`, `data/`, `downloads/` all gitignored). All GitHub work for this project goes through the `Hex-Tech-Lab` org, never a personal account. TypeScript-only, run via `pnpm exec tsx` (npm is disabled on this machine — pnpm only). Test suite: Vitest, `pnpm exec vitest run` (currently covers `payments/src/ledger.ts` and `payments/src/cascade.ts` only — not yet wired into a `package.json` `test` script).
