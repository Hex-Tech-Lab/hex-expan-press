...
RELEASES_DIR="$REPO_ROOT/data/intel/duane_book/releases/samplers"
...
[ -f "$BOOK_DIR/manuscript.md" ] || { echo "missing $BOOK_DIR/manuscript.md" >&2; exit 1; }
...
[ -e "$WORK_DIR/design_rules.json" ] || ln -s "$REPO_ROOT/data/intel/duane_book/qa/design_rules.json" "$WORK_DIR/design_rules.json"
...
# Copy (never touch the canonical manuscript.md), then truncate at end of
# Chapter 2. Locate the Chapter Three chaphead dynamically instead of
# hardcoding a line number.
CH3_MARKER='#chaphead("Chapter Three"'
CH3_LINE="$(grep -n "$CH3_MARKER" "$BOOK_DIR/manuscript.md" | head -1 | cut -d: -f1)"
[ -n "$CH3_LINE" ] || { echo "could not locate Chapter Three chaphead in manuscript.md" >&2; exit 1; }
...
# Keep lines 1..(fence_start - 1). The Chapter Three chaphead sits inside a
...
CUT_LINE="$CH3_LINE"
while [ "$(sed -n "${CUT_LINE}p" "$BOOK_DIR/manuscript.md")" != '```{=typst}' ]; do
  CUT_LINE=$((CUT_LINE - 1))
  [ "$CUT_LINE" -gt 0 ] || { echo "unterminated typst fence before Chapter Three marker" >&2; exit 1; }
done
head -n $((CUT_LINE - 1)) "$BOOK_DIR/manuscript.md" > "$TMP_MANUSCRIPT"

# Truncation sanity check: temp copy must contain ch1+ch2 chapheads but NOT ch3.
for m in 'Chapter One' 'Chapter Two'; do
...
if grep -q "$CH3_MARKER" "$TMP_MANUSCRIPT"; then
  echo "truncation sanity check failed: Chapter Three marker still present in temp copy" >&2
  exit 1
fi
